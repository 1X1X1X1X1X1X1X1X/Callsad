for i,v in pairs(game:GetService("CoreGui"):GetDescendants()) do
	if v.Name == "Motherframe" and v:FindFirstChild("ResizeButton") ~= nil then
		if v.Parent then
			v.Parent:Destroy()
		end
	end
...
function RandomVariable(length)
	...
	for i = 1, length do
		...
		res = res..tostring(math.random(1,9))
	end
	return res
end
maintitle1 = RandomVariable(math.random(1,5))
if not SBot1 then
	SBot1 = "N/A"
end
if not SBot2 then
			...
			function Section:Create(Type, Name, CallBack, Options)
				...
				elseif Type:lower() == "toggle" then 
					...
					Interactables.ToggleShow = Library:Create("ImageLabel", {						
						BackgroundColor3 = Color3.fromRGB(255, 84, 101),
						BackgroundTransparency = 1,
						Position = UDim2.new(0.0359999985, 0, 0.115000002, 0),
						Size = UDim2.new(0, 26, 0, 20),
						Image = "rbxassetid://3570695787",
                        ImageColor3 = Color3.fromRGB(255, 84, 101),
						ScaleType = Enum.ScaleType.Slice,
						SliceCenter = Rect.new(100, 100, 100, 100),
						SliceScale = 0.040,
					})
					...
					Interactables.Toggle.MouseButton1Click:Connect(function()
						...
						if State then 
							TS:Create(Interactables.ToggleShow, TweenInfo.new(0.1), {
                                ImageColor3 = Color3.fromRGB(84, 255, 178),
								Position = UDim2.new(0.53, 0, 0.115, 0)
							}):Play()
						else
							TS:Create(Interactables.ToggleShow, TweenInfo.new(0.1), {
                    ...
                    function Interactables:ChangeState(State)
                        if State then
							...
							TS:Create(Interactables.ToggleShow, TweenInfo.new(0.1), {
                                BackgroundColor3 = Color3.fromRGB(84, 255, 178),
								Position = UDim2.new(0.036, 0, 0.115, 0)
                            }):Play()
                            if CallBack then 
								CallBack(State)
				...
				elseif Type:lower() == "dropdown" then 
					if Options and not Options.search then
						...
						if Options then
                            ...
                            elseif Options.refresh and not Options.playerlist then 
                                tablelist = refreshV
							elseif Options.options and Options.playerlist and not Options.refresh then
								tablelist = {}
					...
					elseif Options and Options.search then
						...
						local function refreshlist()
							...
							print("Refreshed")
                            if Options.refresh then
								if game:GetService("Players").LocalPlayer.PlayerGui.DialogInventory then
									if game:GetService("Players").LocalPlayer.PlayerGui.DialogInventory.Enabled == false then
										game:GetService("Players").LocalPlayer.PlayerGui.DialogInventory.Enabled = true
										game:GetService("Players").LocalPlayer.PlayerGui.DialogInventory.Enabled = false
									end
								end
								ABL = 0
								ABots = {}
								if SBot1 and SBot2 and SBot3 and SBot4 and SBot5 then 
									SBots = {SBot1,SBot2,SBot3,SBot4,SBot5}
									if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("DialogInventory") ~= nil then
										for i,v in pairs (game:GetService("Players").LocalPlayer.PlayerGui.DialogInventory.Frame.ItemsFrameRobo.ItemsFrame.ScrollingFrame:GetDescendants()) do
											if v.Name == "Button" and v.Lock.Visible == true and v.Parent.Name ~= "ItemButton" then
												table.insert(ABots,v.Parent.Name)
												ABL = ABL + 1
											end
										end
										for i,v in next,SBots do
											RV = tostring(v)
											RB = table.find(ABots, RV)
											if RB ~= nil then
												table.remove(ABots, RB)
											end
										end
										tablelist = ABots
									end
